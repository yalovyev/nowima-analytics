...
export default function App() {
  ...
  // Top best practices (highest ocena calls with akcja)
  ...
  // Top worst (lowest ocena with do_poprawy)
          ...
          <>
            ...
            {view === 'calls' && (
              <>
                ...
                {/* Video meetings summary */}
                {meetings.length > 0 && (
                  <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderTop:'3px solid #2B5BDB', borderRadius:12, padding:'16px 20px', marginBottom:24 }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
                      <span style={{ fontFamily:'Outfit', fontWeight:700, fontSize:12, textTransform:'uppercase', letterSpacing:'0.1em', color:'#2B5BDB' }}>🎥 Spotkania wideo</span>
                      <span style={{ fontSize:11, fontFamily:'DM Mono', color:C.text3 }}>{meetings.length} spotkań</span>
                    </div>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
                      {[
                        {l:'Spotkań',v:meetings.length,c:'#2B5BDB'},
                        {l:'Sukces',v:meetings.filter(m=>m.sukces_poziom==='sukces').length,c:C.green},
                        {l:'Częściowy',v:meetings.filter(m=>m.sukces_poziom==='czesciowy').length,c:C.amber},
                        {l:'Śr. %',v:avgPct(meetings)!=null?avgPct(meetings)+'%':'—',c:C.text},
                      ].map((s,i)=>(
                        <div key={i} style={{textAlign:'center',padding:'10px',background:C.surface2,borderRadius:8}}>
                          <div style={{fontFamily:'Outfit',fontWeight:700,fontSize:22,color:s.c}}>{s.v}</div>
                          <div style={{fontSize:10,color:C.text3,fontFamily:'DM Mono',marginTop:2}}>{s.l}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    {topBest.map((c,i) => {
                      ...
                      return(
                        <div key={c.id} style={{display:'flex',alignItems:'flex-start',gap:12,padding:'12px 20px',borderBottom:`1px solid ${C.border}`}}>
                          ...
                          <div style={{fontFamily:'DM Mono',fontWeight:700,fontSize:13,color:C.green}}>{'⭐'.repeat(c.ocena||0)}</div>
                        </div>
                      );
                    })}
                  </div>
                      ...
                      return(
                        <div key={c.id} style={{display:'flex',alignItems:'flex-start',gap:12,padding:'12px 20px',borderBottom:`1px solid ${C.border}`}}>
                          ...
                          <div style={{flex:1}}>
                            ...
                            {c.akcja&&<div style={{fontSize:12,color:C.amber,lineHeight:1.4}}>→ Rekomendacja: {c.akcja}</div>}
                          </div>
                        </div>
                      );
                    })}
